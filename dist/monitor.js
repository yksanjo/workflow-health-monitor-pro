...
class Monitor {
    constructor(platformType) {
        this.platform = (0, platforms_1.createPlatform)(platformType);
        this.analyzer = new analyzer_1.Analyzer();
        this.recommendations = new recommendations_1.RecommendationsEngine();
    }
    ...
    async run(options) {
        ...
        const runsWithJobs = await Promise.all(runs.slice(0, 10).map(async (run) => {
            try {
    ...
    calculateHealthScore(stats, bottlenecks, recommendations) {
        ...
        const overall = Math.round(reliability * 0.4 + durationScore * 0.3 + efficiency * 0.3);
        return {
            overall,
